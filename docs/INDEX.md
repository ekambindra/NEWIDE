# Documentation Index

This repository stores the full implementation plan, prompts, contracts, milestone state, and deployment references.

## Plan and Scope

- [Decision-Complete Plan](./plan/DECISION_COMPLETE_PLAN.md)
- [Milestone Implementation Map](./plan/MILESTONE_MAP.md)
- [Codex-Style UI Migration Task List](./plan/CODEX_UI_MIGRATION_TASKLIST.md)
- [Feature Status Tracker](./FEATURE_STATUS.md)
- [Implementation Progress](./IMPLEMENTATION_PROGRESS.md)

## Prompt Engineering

- [Master Operations Prompt](./MASTER_PROMPT.md)
- [Master Execution Prompt](./prompts/MASTER_EXECUTION_PROMPT.md)
- [Multi-Agent Coordinator Prompt](./prompts/MULTI_AGENT_COORDINATOR_PROMPT.md)

## User Guides

- [Beginner Website Guide](./guides/BEGINNER_WEBSITE_GUIDE.md)

## Architecture and Contracts

- [System Architecture](../ARCHITECTURE.md)
- [API and Type Contracts](./architecture/API_AND_TYPE_CONTRACTS.md)
- [Security Controls](../SECURITY.md)

## Governance and Deployment

- [Public Launch + Deployment](./governance/PUBLIC_DEPLOYMENT_AND_RELEASE.md)
- [Open-Core + Enterprise Distribution](./governance/OPEN_CORE_DISTRIBUTION.md)
- [Desktop Release Pipeline](./governance/RELEASE_PIPELINE.md)
- [Deploy + Testing Runbook](./governance/DEPLOY_AND_TEST_RUNBOOK.md)
- [AI Cross-Chat Handoff Ledger](./governance/AI_CROSS_CHAT_HANDOFF.md)
- [Beta Program and Acceptance](./governance/BETA_PROGRAM_AND_ACCEPTANCE.md)
- [GA Signoff Checklist](./governance/GA_SIGNOFF_CHECKLIST.md)
- [GA Signoff Refresh 2026-02-25](./governance/GA_SIGNOFF_REFRESH_2026-02-25.md)
- [Full System Test Case Matrix](./governance/FULL_TEST_CASE_MATRIX.md)
- [GA Signoff Record v0.1.0](./governance/GA_SIGNOFF_RECORD_2026-02-18_v0.1.0.md)
- [Release Notes v0.1.0](./governance/RELEASE_NOTES_v0.1.0.md)
- [Runbook](../RUNBOOK.md)
- [Roadmap](./ROADMAP.md)

## Repository Metadata

- [Repository Info and Links](./REPOSITORY_INFO.md)
- [Chat Continuity Protocol](../CHAT_CONTINUITY.md)
- [Continuity Log (chat1)](../chat1)
- [Continuity Log (chat2)](../chat2)
- [Continuity Log (chat3)](../chat3)
